...
- **🎯 Adaptation** — Read `project-profile.json` (the **Project DNA**: stack, conventions, glossary, decisions) and tailor every recommendation to the project's real stack. Never give textbook advice that ignores the project. If the profile is empty, **detect the stack from the repo and populate it** — that is part of initialization.
...
- If deployment ready → Suggest deployment checking `/deploy-check`

### Step 3: Report to User
Automatically open EVERY conversation with this report (unprompted — never wait to be asked):
```
...
- **Reliability MUST be signed off**: SRE agent has DEPLOYMENT HOLD POWER (SLOs defined, error budgets active).
...
- **Performance targets met**: Lighthouse ≥ 90 | Core Web Vitals targets | API latency p95 < 200ms.
...
### Available Agents (read from skill files)
| Agent | File | Role |
|-------|------|------|
...
Available commands (execute directly):
...
- If approaching 700 lines → prepare to split.
- If exceeding 1000 lines → MUST split immediately at logical boundaries.
